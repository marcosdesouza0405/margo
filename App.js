    ...
    <Modal visible={visivel} animationType="slide" presentationStyle="pageSheet" onRequestClose={onFechar}>
      <View style={{ flex: 1, backgroundColor: C.bg2 }}>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 24 }}
          ...
          {plano === 'admin' && (
            <View style={s.secao}>
              <Text style={s.secLabel}>🌍 MULTILÍNGUE (beta)</Text>
              <Text style={s.secSub}>Fale em qualquer idioma — a Margo entende e responde na sua língua</Text>
              <View style={{ flexDirection: 'row', gap: 8, marginBottom: 8 }}>
                {[[false,'Desligado'],[true,'Ligado']].map(([v, label]) => (
                  <TouchableOpacity key={String(v)} style={[s.chip, (cfg.multilingueOn || false) === v && s.chipOn]}
                    onPress={() => set('multilingueOn', v)}>
                    <Text style={[s.chipTxt, (cfg.multilingueOn || false) === v && s.chipTxtOn]}>{label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
...
export default function App() {
  ...
  const multilingueRef             = useRef(false); // ref do modo multilíngue
  const gravacaoRef                = useRef(null); // gravação em andamento
  const micEstadoSalvoRef          = useRef(false); // mic estava ligado antes do background
  const ultimoIdiomaRef            = useRef(''); // idioma da última interação multilíngue
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', async (nextState) => {
      ...
      if (nextState === 'active') {
        // Só pausa a wake word se ela estiver LIGADA (PAUSE em serviço morto o cria indevidamente)
        ...
        // Restaura o microfone se estava ligado antes de ir pro background
        if (micEstadoSalvoRef.current && !micAtivoRef.current) {
          console.log('[Mic] Restaurando microfone que estava ativo');
          setTimeout(() => { iniciarMicrofone(); }, 800);
        }

      } else if (nextState === 'background') {
        console.log('[WakeWord] App em background — parando STT');
        // Guarda o estado do mic pra restaurar na volta
        micEstadoSalvoRef.current = micAtivoRef.current;
        try { ExpoSpeechRecognitionModule?.stop(); } catch(e) {}
        // Para gravação multilíngue em andamento
  ...
  useEffect(() => {
    ...
    multilingueRef.current = config.multilingueOn || false;
  ...
  function religarMic() {
    ...
    if (micAtivoRef.current) {
      if (multilingueRef.current) {
        setTimeout(() => { gravarMultilingue(); }, 1000);
  ...
  async function falar(texto) {
    ...
    // Japonês só se for MAIORIA do texto (endereço japonês em frase PT não conta)
    const charsJp = (texto.match(/[\u3040-\u30FF\u4E00-\u9FFF]/g) || []).length;
    const temJapones = charsJp > texto.replace(/\s/g, '').length * 0.4;
    ...
    if (!temJapones) {
      try {
        // Prioridade: idioma detectado pelo Groq > acentos no texto > config
    ...
    try {
      ...
      const { recording } = await Audio.Recording.createAsync(
        ...
        (status) => {
          ...
          if (status.durationMillis > 15000) pararGravacaoEEnviar(); // máximo 15s
        },
        300 // metering a cada 300ms
      );
      gravacaoRef.current = { recording, teveFala: false, silencios: 0 };
    } catch(e) {
      console.log('[Multilingue] Erro ao gravar:', e);
      gravacaoRef.current = null;
    }
  }
  ...
  async function pararGravacaoEEnviar() {
    ...
    try {
      ...
      if (!g.teveFala || !uri) {
        // Sem fala — reinicia o ciclo
        if (micAtivoRef.current && !ttsAtivoRef.current) setTimeout(gravarMultilingue, 300);
        return;
      }
      ...
      const d = await r.json();
      if (d.texto) {
        console.log('[Multilingue]', d.idioma, ':', d.texto);
        ultimoIdiomaRef.current = (d.idioma || '').toLowerCase();
        enviar(d.texto, d.idioma || '');
      } else {
        if (micAtivoRef.current && !ttsAtivoRef.current) setTimeout(gravarMultilingue, 300);
      }
    } catch(e) {
      console.log('[Multilingue] Erro ao enviar:', e);
      if (micAtivoRef.current && !ttsAtivoRef.current) setTimeout(gravarMultilingue, 500);
    }
  }
  ...
  async function iniciarMicrofone() {
    ...
    try {
      ...
      try { await activateKeepAwakeAsync('mic'); } catch(e) {}
      if (multilingueRef.current) {
        gravarMultilingue();
      } else {
  ...
  async function pararMicrofone() {
    micAtivoRef.current = false;
    setMicAtivo(false);
    try { deactivateKeepAwake('mic'); } catch(e) {}

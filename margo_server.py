...
def _pre_detectar(msg: str, hora_local: str = "") -> dict:
    ...
    if any(k in msg for k in all_smart) and (any(d in msg for d in smart_devs) or tem_ar):
        ...
        if tem_hora:
            ...
            disp = _limpar_titulo(disp)
            # ── Delta minutos: tempo relativo pro agendamento (imune a timezone) ──
            _delta_min = tempo.get("minutos_relativos", 0)
            if not _delta_min and hora_local and tempo.get("data_hora_iso"):
                try:
                    _hl = hora_local.replace("Z", "+00:00")
                    if len(_hl) > 5 and _hl[-5] in '+-' and ':' not in _hl[-5:]:
                        _hl = _hl[:-2] + ':' + _hl[-2:]
                    _hora_local_dt = datetime.fromisoformat(_hl)
                    _utc_offset = _hora_local_dt.utcoffset()
                    _dt_utc = datetime.fromisoformat(tempo["data_hora_iso"])
                    _dt_local_alvo = (_dt_utc + _utc_offset) if _utc_offset else _dt_utc
                    _agora_local = _hora_local_dt.replace(tzinfo=None)
                    _delta_min = max(1, int((_dt_local_alvo - _agora_local).total_seconds() / 60))
                except Exception:
                    _delta_min = 0
    ...
    _place_types = {
        "sushi bar":"sushi bar","sushi":"sushi restaurant","ramen":"ramen restaurant",
...
- Buscar lugar próximo: {{"ferramenta":"maps_search","query":"nome correto do lugar"}}
  "acha um mega donki" → {{"ferramenta":"maps_search","query":"Don Quijote"}}
  "tem macdonaldis perto" → {{"ferramenta":"maps_search","query":"McDonald's"}}
  "farmácia mais perto" → {{"ferramenta":"maps_search","query":"farmácia"}}
  Use o nome REAL do lugar no query, não o que o usuário digitou/falou literalmente.
...
REGRAS para música:
- Se o usuário pedir algo específico, use exatamente o que pediu no query.
- Se o usuário pedir algo genérico ("toca uma música", "coloca algo"), use a preferência musical do perfil dele.
- Se o usuário pedir pro assistente ESCOLHER, coloque no query o nome da MÚSICA + ARTISTA específico (nunca só o gênero).
...
- NÃO emita JSON de ferramenta agora — a navegação só acontece se o usuário confirmar."""
    ...
    # ── SMART HOME AGENDADO (tempo relativo — imune a timezone) ────────────────
    if ferramenta and ferramenta.get("ferramenta") == "smart_home_agendado":
        delta = ferramenta.get("delta_minutos", 0) or ferramenta.get("minutos_relativos", 0)
        # Fallback: se veio do LLM sem delta_minutos, recalcula
        if not delta and hora_local:
            try:
                _tempo_re = _parsear_tempo(mensagem.lower().strip(), hora_local)
                delta = _tempo_re.get("minutos_relativos", 0)
                if not delta and _tempo_re.get("data_hora_iso"):
                    _hl2 = hora_local.replace("Z", "+00:00")
                    if len(_hl2) > 5 and _hl2[-5] in '+-' and ':' not in _hl2[-5:]:
                        _hl2 = _hl2[:-2] + ':' + _hl2[-2:]
                    _hl2_dt = datetime.fromisoformat(_hl2)
                    _off2 = _hl2_dt.utcoffset()
                    _dt2 = datetime.fromisoformat(_tempo_re["data_hora_iso"])
                    _alvo2 = (_dt2 + _off2) if _off2 else _dt2
                    delta = max(1, int((_alvo2 - _hl2_dt.replace(tzinfo=None)).total_seconds() / 60))
            except Exception as e:
                log(f"Erro recalculando delta smart: {e}", "smart")
                delta = 0
        if delta and int(delta) > 0:
            # Agenda por tempo relativo: now() + delta — sem conversão de timezone
            data_hora_sh = (datetime.now() + timedelta(minutes=int(delta))).strftime("%Y-%m-%dT%H:%M:%S")
            ...
            log(f"Smart agendado (delta {delta}min): {titulo_sh} @ {data_hora_sh} (server time)", "smart")
        else:
            log(f"Smart agendado DESCARTADO (delta=0): {ferramenta}", "smart")
...
async def mensagem(request: Request):
    try:
        ...
        # Só conta interação se a resposta foi real (não fallback/erro)
        resposta_txt = (resultado.get("resposta", "") or "").strip().lower()
        _fallbacks = [
            "desculpe, não entendi", "sorry, i didn't quite understand",
            "すみません、よく理解できませんでした",
            "pode reformular", "could you try again", "もう一度お願い",
            "não consegui entender", "sem resposta",
        ]
        eh_fallback = any(f in resposta_txt for f in _fallbacks)
        eh_muito_curta = len(resposta_txt) < 5
        if not eh_fallback and not eh_muito_curta:
            banco.registrar_uso(user_id, usando_extras=uso.get("usando_extras", False))
        else:
            log(f"Interação NÃO contada (fallback/curta): user={user_id}", "uso")
...
async def recompensa_ad(request: Request):
    ...
    try:
        ...
        try:
            c.execute("ALTER TABLE uso_diario ADD COLUMN ads_assistidos INTEGER DEFAULT 0")
            conn.commit()
        except Exception:
